...
public class admindunkin extends JFrame implements ActionListener
{
	Connection con;
	Statement st;
	ResultSet rs;
	...
	public admindunkin()
	{
		...
		JLabel background=new JLabel(new ImageIcon("G:\\pics\\dunkin2.jpg"));
		add(background);
		...
		try
		{
			rs=st.executeQuery("select * from dunkin");
			while(rs.next())
			{
	...
	public void actionPerformed(ActionEvent ar) 
	{
		...
		if(ar.getSource()==b1)
		{
			try
			{
				rs=st.executeQuery("select * from dunkin where id='"+c1.getSelectedItem()+"'");
				while(rs.next())
				{
		...
		if(ar.getSource()==b2)
		{
			try
			{
				st.executeUpdate("insert into dunkin values('"+t1.getText()+"','"+t2.getText()+"','"+t3.getText()+"','"+t4.getText()+"')");
		...
		if(ar.getSource()==b3)
		{
			try
			{
				st.executeUpdate("delete from dunkin where id='"+c1.getSelectedItem()+"'");
		...
		if(ar.getSource()==b4)
		{
			try
			{
				st.executeUpdate("update dunkin set Category='"+t2.getText()+"',Product='"+t3.getText()+"',Price='"+t4.getText()+"' where id='"+c1.getSelectedItem()+"'");
