...
public class barista extends JFrame implements ActionListener
{
	float sub;
	...
	public barista()
	{
		...
		JLabel background=new JLabel(new ImageIcon("G:\\pics\\barista2.jpg"));
		add(background);
		back.setBounds(10,10,100,30);background.add(back);
		b0.setBounds(610,465,130,40);background.add(b0);
		l1.setBounds(425,530,120,20);background.add(l1);
		l1.setFont(new Font("Comic Sans MS", Font.PLAIN, 15));
		t1.setBounds(555,530,100,20);background.add(t1);
		l2.setBounds(690,530,120,20);background.add(l2);
	    l2.setFont(new Font("Comic Sans MS", Font.PLAIN, 15));
	    t2.setBounds(805,530,100,20);background.add(t2);
		b1.setBounds(560,580,100,25);background.add(b1);
		b2.setBounds(690,580,100,25);background.add(b2);
		l3.setBounds(595,630,100,25);background.add(l3);
		l3.setFont(new Font("Comic Sans MS", Font.PLAIN, 15));
		l4.setBounds(680,630,100,25);background.add(l4);
	...
	public void actionPerformed(ActionEvent ar) 
	{
		...
		if(ar.getSource()==b0)
		{
			JOptionPane.showMessageDialog(null,"Sorry. No offers available at Barista.");
		}
		if(ar.getSource()==b1)
		{
			...
			try
			{
				rs=smt.executeQuery("select * from barista where id='"+t1.getText()+"'");
				if(rs.next())
				{
