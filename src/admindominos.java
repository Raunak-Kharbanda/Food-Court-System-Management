...
public class admindominos extends JFrame implements ActionListener
{
	Connection con;
	Statement st;
	ResultSet rs;
	...
	public admindominos()
	{
		...
		JLabel background=new JLabel(new ImageIcon("G:\\pics\\dominos2.jpg"));
		add(background);
		back.setBounds(5,10,70,30);background.add(back);
		...
		try
		{
			rs=st.executeQuery("select * from dominos");
			while(rs.next())
			{
	...
	public void actionPerformed(ActionEvent ar) 
	{
		...
		if(ar.getSource()==b1)
		{
			try
			{
				rs=st.executeQuery("select * from dominos where id='"+c1.getSelectedItem()+"'");
				while(rs.next())
				{
		...
		if(ar.getSource()==b2)
		{
			try
			{
				st.executeUpdate("insert into dominos values('"+t1.getText()+"','"+t2.getText()+"','"+t3.getText()+"','"+t4.getText()+"')");
		...
		if(ar.getSource()==b3)
		{
			try
			{
				st.executeUpdate("delete from dominos where id='"+c1.getSelectedItem()+"'");
		...
		if(ar.getSource()==b4)
		{
			try
			{
				st.executeUpdate("update dominos set Category='"+t2.getText()+"',Product='"+t3.getText()+"',Price='"+t4.getText()+"' where id='"+c1.getSelectedItem()+"'");
