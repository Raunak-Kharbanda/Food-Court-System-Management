...
public class Login extends JFrame implements ActionListener
{
	...
	public Login()
	{
		...
		b2.setBounds(585,450,150,30);background.add(b2);
	...
	public void actionPerformed(ActionEvent ar) 
	{
		if(ar.getSource()==b1)
		{
			try
			{
				rs=st.executeQuery("select * from cashier where UName='"+t1.getText()+"' and Password='"+t2.getText()+"'");
				if(rs.next())
				{
					UName=t1.getText();
					new ChainList(UName);
		...
		if(ar.getSource()==b2)
		{
			...
			if(rs.next())
			{
				st.executeUpdate("delete from cashier where UName='"+t1.getText()+"' and Password='"+t2.getText()+"'");
				JOptionPane.showMessageDialog(null,"Cashier account deleted.");
				this.hide();
			}
			else
			{
